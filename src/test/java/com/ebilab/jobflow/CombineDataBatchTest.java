...
public class CombineDataBatchTest {
	...
	public void test() {
		...
		tester.jobflow("CombineDataJob").output("out", DayWordCountModel.class).verify(out + "#output", out + "#rule");
		tester.jobflow("CombineDataJob").output("outDispTfIdf", DispDayWordModel.class).verify(outDispTfIdf + "#output", outDispTfIdf + "#rule");
